			...
			<table class="channel-table" border="1">
				...
				<tbody>
					<c:forEach var="subscriber" items="${subscriberList}">
						<tr align="center">
							<td
								onclick="location.href='userInfo?id=${member.id }&currentPage=${currentPage }'">${subscriber.id}</td>
							<td>${subscriber.registration_Date}</td>
							<td>${subscriber.subscriber_Count}명</td>
							<td><c:choose>
									<c:when test="${subscriber.is_Subscribed}">
										<!-- 구독 중인 경우 -->
										<span class="subscription-btn"
											data-subscriber-id="${subscriber.id}"
											data-action="unsubscribe" onclick="toggleSubscription(this)">&#x2764;</span>
									</c:when>
									<c:otherwise>
										<!-- 구독하지 않은 경우 -->
										<span class="subscription-btn"
											data-subscriber-id="${subscriber.id}" data-action="subscribe"
