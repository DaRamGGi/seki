		...
		<div class="viewAnnounce">
			...
			<div class="fileDiv">
				<c:choose>
					...
					<c:otherwise>
						<p onclick="fileDownload(${inquiry.num })">
							파일 다운로드 > <span>${inquiry.files }</span>
						</p>
					</c:otherwise>
				</c:choose>
			</div>
			<c:if
				test="${sessionScope.id == inquiry.writer || sessionScope.id == 'admin'}">
				...
				<div class="adminReply">
					<h2>
						...
						<c:if test="${sessionScope.id == 'admin'}">
							<a href="#" id="settings-${inquiryReply.replyNum}"> <span
								class="material-symbols-sharp">settings</span>
							</a>
						</c:if>
						<ul id="clickSettings-${inquiryReply.replyNum}"
							class="clickSettings">
							<li
								onclick="modifyAdminReplyClick(${inquiryReply.replyNum}, ${inquiryReply.centerNum})">수정</li>
							<li>|</li>
							<li
								onclick="deleteAdminReply(${inquiryReply.replyNum}, centerNum=${inquiryReply.centerNum})">삭제</li>
						</ul>
					</h2>
					<hr>
