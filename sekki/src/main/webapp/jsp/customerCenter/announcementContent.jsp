		...
		<div class="viewAnnounce">
			...
			<c:choose>
				<c:when test="${sessionScope.id == 'admin'}">
					<p class="settingBox">
						<span class="material-symbols-outlined">account_circle</span> <span>${announcement.writer }</span>
			...
			<ul class="clickSetting">
				<li
					onclick="location.href='modifyAnnouncement?num=${announcement.num }'">수정</li>
				<li>|</li>
				<li onclick="deleteAnnouncementProc(${announcement.num})">삭제</li>
			</ul>
			<p>${announcement.writeDate }${announcement.writeTime }</p>
			<p>
				<span style="font-weight: bold;">조회수</span> ${announcement.hits }
			</p>
			<hr>
			...
			<div class="fileDiv">
				<c:choose>
					...
					<c:otherwise>
						<p onclick="fileDownload(${announcement.num })">
							파일 다운로드 > <span>${announcement.files }</span>
						</p>
					</c:otherwise>
				</c:choose>
			</div>
				...
				<li><c:choose>
						<c:when test="${isLiked == 1}">
							<span class="heart heart-filled"
								onclick="clickHeart(${announcement.num})">&#10084;</span>
						</c:when>
						<c:otherwise>
			...
			<div class="announceReply">
				<ul class="replyCountBox">
					<li><h2>
							<span class="material-symbols-sharp">chat</span> 댓글 &nbsp;
						</h2></li>
				...
				<table class="announceReplyTable">
					<thead>
						<tr class="announceWriteReply">
							<td><input type="text" placeholder="댓글 작성(최대 200자)"
								id="reply" maxlength="200" />
								<button onclick="writeAnnouncementReply(${announcement.num })">작성</button>
							</td>
						</tr>
						<tr>
							<td>내 댓글만 보기 <input type="checkbox" id="myReplyOnly"
								onclick="myReplyOnly(${announcement.num })" />
							</td>
						</tr>
					</thead>
						...
						<c:forEach var="announcementReply" items="${announcementReplys }">
							<tr>
								<td class="myReplyTd">
									...
									<p class="writeInfo">${announcementReply.writer }
										${announcementReply.writeDate }</p>
								</td>
							</tr>
						</c:forEach>
					</tbody>
